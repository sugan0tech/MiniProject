...
public class MembershipRepoTests
{
    ...
    public async Task GetAll_ShouldReturnAllEntities()
    {
        ...
        await _context.Memberships.AddRangeAsync(
            ...
            {
                Type = MemberShip.PremiumUser.ToString(), ProfileId = 1, Description = "Premium membership",
                EndsAt = DateTime.Now.AddMonths(1), IsTrail = false
            },
            new MatrimonyApiService.Membership.Membership
            {
                Type = MemberShip.BasicUser.ToString(), ProfileId = 2, Description = "Basic membership",
                EndsAt = DateTime.Now.AddMonths(1), IsTrail = true
            }
        );
        await _context.SaveChangesAsync();
    ...
    public async Task Add_ShouldAddEntity()
    {
        ...
        var membership = new MatrimonyApiService.Membership.Membership
        {
            Type = "Premium", ProfileId = 1, Description = "Premium membership",
            EndsAt = DateTime.Now.AddMonths(1), IsTrail = false
        };
    ...
    {
        // Arrange
        var updateMembership = new MatrimonyApiService.Membership.Membership
        {
            Id = 99, Type = "Basic", ProfileId = 1, Description = "Non-existent membership",
            EndsAt = DateTime.Now.AddMonths(1), IsTrail = true
        };

        // Act & Assert
        var ex = Assert.ThrowsAsync<KeyNotFoundException>(async () =>
            await _membershipRepo.Update(updateMembership));
        ClassicAssert.AreEqual("Membership with key 99 not found!!!", ex.Message);
    }
